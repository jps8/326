    ...
    | Some (nextLink, rest) -> (
      ...
      | Some page -> (
        ...
        let rec listToDiffSet l setToRemove = 
          ...
          | [] -> LinkSet.empty
          | hd::tl -> (
            if LinkSet.member setToRemove hd then 
              (listToDiffSet tl setToRemove)
            else 
              LinkSet.insert hd (listToDiffSet tl setToRemove)
          )
        in
        ...
        let rec updatedDict oldDict (link:LinkSet.elt) wordList = 
          ...
          | hd::tl -> (
            let inserted =
              ...
              | Some assocLinks -> 
                WordDict.insert oldDict hd (LinkSet.insert link assocLinks)
            in
